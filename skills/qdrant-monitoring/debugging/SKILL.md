...
- Use `/collections/{collection_name}/optimizations` endpoint (v1.17+) to check status [Optimization monitoring](/md/documentation/ops-optimization/optimizer/?s=optimization-monitoring)
...
- Web UI has an Optimizations tab with timeline view and per-task duration metrics [Web UI](/md/documentation/ops-optimization/optimizer/?s=web-ui)
...
- Estimate expected memory: `num_vectors * dimensions * 4 bytes * 1.5` for vectors, plus payload and index overhead [Capacity planning](/md/documentation/capacity-planning/)
...
- Compare filtered vs unfiltered query times. Large gap means missing payload index. [Payload index](/md/documentation/manage-data/indexing/?s=payload-index)
