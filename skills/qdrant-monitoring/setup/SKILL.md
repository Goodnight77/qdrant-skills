...
- Node metrics at `/metrics` endpoint [Monitoring docs](/md/documentation/ops-monitoring/monitoring/)
...
- Hybrid Cloud Prometheus setup tutorial [Hybrid Cloud Prometheus](/md/documentation/ops-monitoring/hybrid-cloud-prometheus/)
...
- Use `/healthz`, `/livez`, `/readyz` for basic status, liveness, and readiness [Kubernetes health endpoints](/md/documentation/ops-monitoring/monitoring/?s=kubernetes-health-endpoints)
...
- Hybrid Cloud provides ~11 pre-configured Prometheus alerts out of the box [Cloud cluster monitoring](/md/documentation/cloud/cluster-monitoring/)
...
- Enable JSON log format for structured analysis: set `logger.format` to `json` in config [Configuration](/md/documentation/ops-configuration/configuration/)
- Use FluentD/OpenSearch for log aggregation
- Audit logs (v1.17+) write to local filesystem (`/qdrant/storage/audit/`), not stdout. Mount a Persistent Volume and deploy a sidecar container to tail these files to stdout so DaemonSets can pick them up. [Audit logging](/md/documentation/security/?s=audit-logging)
