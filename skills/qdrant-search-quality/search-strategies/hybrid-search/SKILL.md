...
Prefetches can be nested and searches can be multi-stage, all pipeline happening in one request through Query API. See [Universal Query API](/md/course/essentials/day-5/universal-query-api/) for examples.
...
1. Configure Qdrant collection with [named vectors](/md/documentation/manage-data/vectors/?s=named-vectors), where each named vector usually corresponds to one representation (different embedding models or different vector types) of a data point.
2. Construct a hybrid search request with Query API from your building blocks. You can search independently among one type of vectors, with `prefetch` + `using`, like shown in examples in [Hybrid Queries documentation](/md/documentation/search/hybrid-queries).
...
- Indexes (sparse, payload and dense) and [IDF modifier](/md/documentation/manage-data/indexing/?s=idf-modifier) for sparse vectors are computed independently **per shard**, not per **tenant**.
...
- Choose a hybrid search pattern based on "vibes" without any [hybrid search quality evaluation](/md/articles/hybrid-search/?s=how-effective-is-your-search-system) in-place.
