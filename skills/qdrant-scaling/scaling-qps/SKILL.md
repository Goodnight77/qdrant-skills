...
- Use fewer, larger segments (`default_segment_number: 2`) [Maximizing throughput](/md/documentation/ops-optimization/optimize/?s=maximizing-throughput)
- Enable quantization with `always_ram=true` to reduce disk IO [Quantization](/md/documentation/manage-data/quantization/)
- Use batch search API to amortize overhead [Batch search](/md/documentation/search/search/?s=batch-search-api)

## Minimize impact of Update Workloads

- Configure update throughput control (v1.17+) to prevent unoptimized searches degrading reads [Low latency search](/md/documentation/search/low-latency-search/)
- Set `optimizer_cpu_budget` to limit indexing CPUs (e.g. `2` on an 8-CPU node reserves 6 for queries)
- Configure delayed read fan-out (v1.17+) for tail latency [Delayed fan-outs](/md/documentation/search/low-latency-search/?s=use-delayed-fan-outs)
...
- Use `replication_factor: 2+` and route reads to replicas [Distributed deployment](/md/documentation/distributed_deployment/?s=replication)
...
- In case of quantized vectors, prefer global rescoring over per-segment rescoring to reduce disk reads. Example in the [tutorial](/md/documentation/tutorials-operations/large-scale-search/?s=search-query)
- Configure higher number of search threads to parallelize disk reads. Default is `cpu_count - 1`, which is optimal for RAM-based search but may be too low for disk-based search. See [configuration reference](/md/documentation/ops-configuration/configuration/?s=configuration-options)
