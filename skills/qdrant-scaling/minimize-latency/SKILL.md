...
- Increase segment count to match CPU cores (`default_segment_number: 16`) [Minimizing latency](/md/documentation/ops-optimization/optimize/?s=minimizing-latency)
- Keep quantized vectors and HNSW in RAM (`always_ram=true`)
- Reduce `hnsw_ef` at query time (trade recall for speed) [Search params](/md/documentation/ops-optimization/optimize/?s=fine-tuning-search-parameters)
...
- Use quantization: scalar (4x reduction) or binary (16x reduction) [Quantization](/md/documentation/manage-data/quantization/)
- Move payload indexes to disk if filtering is infrequent [On-disk payload index](/md/documentation/manage-data/indexing/?s=on-disk-payload-index)
