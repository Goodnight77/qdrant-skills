...
Use when: data has natural time boundaries (daily, weekly, monthly). Preferred because queries span all time periods in one request without application-level fan-out. [User-defined sharding](/md/documentation/distributed_deployment/?s=user-defined-sharding)
...
Use when: you need per-period collection configuration (e.g., different quantization or storage settings). [Collection aliases](/md/documentation/manage-data/collections/?s=collection-aliases)
...
3. When a new period starts, create the new collection and swap the write alias [Switch collection](/md/documentation/manage-data/collections/?s=switch-collection)
...
1. Store a `timestamp` payload on every point, create a payload index on it [Payload index](/md/documentation/manage-data/indexing/?s=payload-index)
2. Filter to the desired window at query time using `range` condition [Range filter](/md/documentation/search/filtering/?s=range)
3. Periodically delete expired points using delete-by-filter [Delete points](/md/documentation/manage-data/points/?s=delete-points)
...
- **Collection rotation:** keep current collection in RAM (`always_ram: true`), move older collections to mmap/on-disk vectors. [Quantization](/md/documentation/manage-data/quantization/)
