...
- Tune HNSW parameters: [Fine-tuning search](/md/documentation/ops-optimization/optimize/?s=fine-tuning-search-parameters)
- Enable in-memory quantization: [Scalar quantization](/md/documentation/manage-data/quantization/?s=scalar-quantization)
- Reduce Vector Dimensionality with Matryoshka Models: [Matryoshka Models](/md/documentation/inference/?s=reduce-vector-dimensionality-with-matryoshka-models)
- Use oversampling + rescore for high-dimensional vectors [Search with quantization](/md/documentation/manage-data/quantization/?s=searching-with-quantization)
...
- Reduce segment count (`default_segment_number` to 2) [Maximizing throughput](/md/documentation/ops-optimization/optimize/?s=maximizing-throughput)
- Use batch search API instead of single queries [Batch search](/md/documentation/search/search/?s=batch-search-api)
- Enable quantization to reduce CPU cost [Scalar quantization](/md/documentation/manage-data/quantization/?s=scalar-quantization)
- Add replicas to distribute read load [Replication](/md/documentation/distributed_deployment/?s=replication)
...
- Create payload index on the filtered field [Payload index](/md/documentation/manage-data/indexing/?s=payload-index)
- Use `is_tenant=true` for primary filtering condition: [Tenant index](/md/documentation/manage-data/indexing/?s=tenant-index)
- Try ACORN algorithm for complex filters: [ACORN](/md/documentation/search/search/?s=acorn-search-algorithm)
...
Learn more [here](/md/documentation/search/low-latency-search/?s=query-indexed-data-only)
