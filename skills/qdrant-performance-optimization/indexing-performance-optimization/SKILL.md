...
- Understand the indexing optimizer [Indexing optimizer](/md/documentation/ops-optimization/optimizer/?s=indexing-optimizer)
...
- Use batch upserts (64-256 points per request) [Points API](/md/documentation/manage-data/points/?s=upload-points)
- Use 2-4 parallel upload streams

For server-side, optimize Qdrant configuration and indexing strategy:

- Create more shards (3-12), each shard has an independent update worker [Sharding](/md/documentation/distributed_deployment/?s=sharding)
- Create payload indexes before HNSW builds (needed for filterable vector index) [Payload index](/md/documentation/manage-data/indexing/?s=payload-index)

Suitable for initial bulk load of large datasets:

- Disable HNSW during bulk load (set `indexing_threshold_kb` very high, restore after) [Collection params](/md/documentation/manage-data/collections/?s=update-collection-parameters)
...
See /md/documentation/tutorials-develop/bulk-upload/
...
- Check actual progress via optimizations endpoint (v1.17+) [Optimization monitoring](/md/documentation/ops-optimization/optimizer/?s=optimization-monitoring)
...
- Reduce `m` (default 16, good for most cases, 32+ rarely needed) [HNSW params](/md/documentation/manage-data/indexing/?s=vector-index)
- Reduce `ef_construct` (100-200 sufficient) [HNSW config](/md/documentation/manage-data/collections/?s=indexing-vectors-in-hnsw)
- Keep `max_indexing_threads` proportional to CPU cores [Configuration](/md/documentation/ops-configuration/configuration/)
- Use GPU for indexing [GPU indexing](/md/documentation/ops-configuration/running-with-gpu/)
...
See [Multi-tenant collections](/md/documentation/manage-data/multitenancy/) for details.
...
Read more about disabling extra HNSW links in [documentation](/md/documentation/manage-data/indexing/?s=disable-the-creation-of-extra-edges-for-payload-fields)

Read more about ACORN in [documentation](/md/documentation/search/search/?s=acorn-search-algorithm)
