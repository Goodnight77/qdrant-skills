...
- Qdrant exposes memory usage through the `/metrics` endpoint. See [Monitoring docs](/md/documentation/ops-monitoring/monitoring/).
...
- For regular search scenarios, general guidelines are provided in the [Capacity planning docs](/md/documentation/capacity-planning/).

For a detailed breakdown of memory usage at large scale, see [Large scale memory usage example](/md/documentation/tutorials-operations/large-scale-search/?s=memory-usage).
...
- For deployments with [inline storage](/md/documentation/ops-optimization/optimize/?s=inline-storage-in-hnsw-index) enabled.
...
- Use quantization to store only compressed vectors in RAM [Quantization docs](/md/documentation/manage-data/quantization/)

- Use float16 or int8 datatypes to reduce memory usage of vectors by 2x or 4x respectively, with some tradeoff in precision. Read more about vector datatypes in [documentation](/md/documentation/manage-data/vectors/?s=datatypes)

- Leverage Matryoshka Representation Learning (MRL) to store only small vectors in RAM while keeping large vectors on disk. Examples of how to use MRL with Qdrant Cloud inference: [MRL docs](/md/documentation/inference/?s=reduce-vector-dimensionality-with-matryoshka-models)

- For multi-tenant deployments with small tenants, vectors might be stored on disk because the same tenant's data is stored together [Multitenancy docs](/md/documentation/manage-data/multitenancy/?s=calibrate-performance)
...
- Configure payload indexes to be stored on disk [docs](/md/documentation/manage-data/indexing/?s=on-disk-payload-index)
- Configure sparse vectors to be stored on disk [docs](/md/documentation/manage-data/indexing/?s=sparse-vector-index)
